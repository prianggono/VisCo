...
export function validateRelationships(graph: RelationshipGraph): RelationshipValidationResult {
  ...
  for (const composition of graph.compositions) {
    ...
    for (const groupId of composition.groupIds) {
      if (!groupIds.has(groupId)) errors.push(`Composition "${composition.id}" references missing group "${groupId}".`);
    }
  ...
  for (const deck of graph.decks) {
    if (deck.compositionId !== undefined && !compositionIds.has(deck.compositionId)) {
      errors.push(`Deck "${deck.id}" references missing composition "${deck.compositionId}".`);
    }
    for (const layer of deck.layers) {
      if (!layerIds.has(layer.id)) errors.push(`Deck "${deck.id}" contains unregistered layer "${layer.id}".`);
      if (layer.sourceId !== undefined && !layer.sourceId.trim()) {
        errors.push(`Layer "${layer.id}" has an empty source reference.`);
      }
  ...
  for (const group of graph.groups) {
    for (const layerId of group.layerIds) {
      if (!layerIds.has(layerId)) errors.push(`Group "${group.id}" references missing layer "${layerId}".`);
    }
  }

  for (const slice of graph.slices) {
    for (const layerId of slice.layerIds) {
      if (!layerIds.has(layerId)) errors.push(`Slice "${slice.id}" references missing layer "${layerId}".`);
    }
  }
